...
from app.schemas.task import TaskCreate, TaskUpdate, TaskOut, TaskRead
...
async def read_tasks(db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
async def update_existing_task(task_id: int, task: TaskUpdate, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
async def delete_existing_task(task_id: int, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
